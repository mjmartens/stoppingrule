#' @title The Inverse Function of the Stopping Rule Boundary (Binary Data)
#' @description Calculate the inverse of the boundary for a given stopping rule
#'
#' @param b Number of toxicities
#' @param n Sample size
#' @param p0 The toxicity probability under the null hypothesis
#' @param type The method used for constructing the stopping rule. Choices include a Pocock test ("Pocock"), an O'Brien-Fleming test ("OBF"), a Wang-Tsiatis test ("WT"), the Bayesian beta-binomial method ("BB") proposed by Geller et al. 2003, a truncated SPRT ("SPRT"), and a maximized SPRT ("MaxSPRT").
...
#'
#' @return A univariate function that defines the effective sample size value corresponding to the number of toxicities (used in TITE method)
#' @export

bdryfcn.bin.inverse = function(b,n,p0,type,cval,param=NULL) {
  if(type=="Pocock") {
    val = function(x) {
